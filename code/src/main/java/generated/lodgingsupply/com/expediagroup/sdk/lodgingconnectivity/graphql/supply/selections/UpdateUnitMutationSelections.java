...
import com.expediagroup.sdk.lodgingconnectivity.graphql.supply.type.UpdatePropertyUnitPayload;
import java.util.Arrays;
import java.util.List;

public class UpdateUnitMutationSelections {
  private static List<CompiledSelection> __unit = Arrays.asList(
    new CompiledField.Builder("id", new CompiledNotNullType(GraphQLID.type)).build()
  );

  private static List<CompiledSelection> __updateUnit = Arrays.asList(
    new CompiledField.Builder("clientMutationId", GraphQLString.type).build(),
    new CompiledField.Builder("unit", new CompiledNotNullType(Unit.type)).selections(__unit).build()
  );

  public static List<CompiledSelection> __root = Arrays.asList(
    new CompiledField.Builder("updateUnit", UpdatePropertyUnitPayload.type).arguments(Arrays.asList(new CompiledArgument.Builder(Mutation.__updateUnit_input).value(new CompiledVariable("input")).build())).selections(__updateUnit).build()
  );
}
